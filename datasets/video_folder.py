...
def make_dataset(quality, mode):
    ...
    assert mode in ['train', 'validation', 'test', 'test_easy', 'test_hard', 'video_folder']
    ...
    for img_token in c_tokens:
        all_tokens.append(osp.join(image_path, img_token))
    logging.info(f'Current burst has a total of {len(all_tokens)} images in {mode} phase')

    logging.info(f'Infer-{mode}: {len(all_tokens)} images')

    return all_tokens


class InferBurst(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.joint_transform_list is not None:
            for _, xform in enumerate(self.joint_transform_list):
                image = xform(image)

        if self.transform is not None:
            image = self.transform(image)

        return image, image_name # Note that we only output the image itself and its name from this getitem method
